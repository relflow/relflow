...
def test_schema_pickle_discards_derived_selection_predicates():
    ...
    assert selected == [model.schema.requests["/id"]]
    assert restored._selection_cache == {}
    assert restored.select(rf.where("type") == "category") == [restored.requests["/id"]]
...
def test_model_uses_constructor_without_from_tree_alternative():
    ...
    assert model.schema.requests["/amount"].name == "amount"
...
def test_model_constructor_accepts_branch_nodes_with_optional_leaf_queries():
    ...
    assert "/transactions" in params.branches

    amount = params.requests["/transactions/amount"]
    assert amount.query is None
    assert params.shapes["/transactions/amount"] == (1, 4)

    merchant = params.requests["/transactions/merchant_code"]
...
def test_branch_mask_shorthand_normalizes_and_exports_public_api():
    ...
    bound = model.schema.branches["/transactions"]
...
def test_model_constructor_accepts_root_mask():
    ...
    assert model.schema.reconstruct == ["/amount"]
...
def test_model_select_returns_nodes_and_update_refreshes_cached_role_views():
    ...
    assert params.requests["/amount"].weight == 2.0

    model.update(rf.where("name") == "amount", description="Transaction amount")
    assert model.select(rf.where("description") == "Transaction amount") == [params.requests["/amount"]]

    reconstruct = rf.where("reconstruct")
    assert model.select(reconstruct, include_root=False) == [params.requests["/label"]]

    model.update(rf.where("name") == "amount", mask=True)
    assert params.requests["/amount"].mask == (rf.Mask(skip=True, dropout=False, reconstruct=True),)
    assert model.select(reconstruct, include_root=False) == [
        params.requests["/amount"],
        params.requests["/label"],
    ]

    model.update(rf.where("name") == "amount", mask=False)
    assert params.requests["/amount"].mask == ()
    assert model.select(reconstruct, include_root=False) == [params.requests["/label"]]
...
def test_schema_select_returns_nodes_and_accepts_boolean_predicates():
    ...
    assert active == [params.requests["/amount"]]
    assert inactive == [params.requests["/memo"]]

    model.update(rf.where("name") == "memo", mask=True)
    assert params.requests["/memo"].mask == (rf.Mask(skip=True, dropout=False, reconstruct=True),)
...
def test_model_update_can_deactivate_and_reactivate_leaf_nodes():
    ...
    assert "/memo" in params.requests
    assert "/memo" not in params.active_requests
    assert "/memo" in model.nodes
    assert params.embed == []
    inactive = model.select(lambda node: getattr(node, "active", True) is False)
    assert inactive[0].address == "/memo"

    model.update(rf.where("name") == "memo", active=True)

    assert "/memo" in params.requests
    assert "/memo" in params.active_requests
    assert "/memo" in model.nodes
    assert params.embed == ["/memo"]
...
def test_model_update_uses_current_schema_when_selection_cache_is_stale():
    ...
    assert model.select(predicate) == [model.schema.requests["/amount"]]

    request = model.schema.requests["/amount"]
    request.name = "renamed"

    model.update(predicate, weight=2.0)

    assert request.weight == 1.0
    assert "/amount" not in model.schema.requests
    assert "/renamed" in model.schema.requests
    assert "/amount" not in model.nodes
    assert "/renamed" in model.nodes
...
def test_model_extend_appends_fields_under_one_selected_array_and_rebuilds_modules():
    ...
    model.extend(rf.where("address") == "/transactions", risk_score=rf.Number())

    assert "/transactions/risk_score" in params.requests
    assert "/transactions/risk_score" in model.nodes
...
def test_model_extend_appends_category_field_and_preserves_existing_vocabulary():
    ...
    label_vocab = model.nodes["/label"].embedder.vocab
    label_vocab.extend(["alpha", "beta"])

    model.extend(rf.where("address") == "/", caretaker=rf.Category())

    assert "/caretaker" in model.schema.requests
    assert "/caretaker" in model.nodes
    assert model.nodes["/label"].embedder.vocab.snapshot() == ["alpha", "beta"]
    assert model.nodes["/caretaker"].embedder.vocab.snapshot() == []
...
def test_model_extend_defaults_to_root_when_only_one_array_matches():
    ...
    assert "/risk_score" in model.schema.requests
    assert "/risk_score" in model.nodes
...
def test_model_delete_removes_nodes_permanently_and_rebuilds_modules():
    ...
    assert "/risk_score" not in params.requests
    assert "/risk_score" not in model.nodes
...
def test_model_reset_reinitializes_runtime_node_without_changing_schema():
    model = rf.Model(amount=rf.Number(), d_model=16, n_layers=1, n_heads=4)
    before = model.nodes["/amount"]

    model.reset(rf.where("name") == "amount")

    assert model.nodes["/amount"] is not before
    assert "/amount" in model.schema.requests
...
def test_model_override_temporarily_updates_schema_and_rebuilds_modules():
    ...
    with model.override(rf.where("name") == "amount", active=False):
        assert "/amount" not in model.schema.active_requests
        assert model.nodes["/amount"] is not before

    assert "/amount" in model.schema.active_requests
...
def test_model_mutations_are_blocked_inside_training_loop_lock():
    ...
    model.update(rf.where("name") == "amount", weight=2.0)
    assert model.schema.requests["/amount"].weight == 2.0
