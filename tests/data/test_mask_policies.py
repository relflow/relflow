...
def test_mask_configuration_matrix(
    query: str | None,
    rate: float | None,
    selection: str,
    skip: bool,
...
def test_arrow_preprocessor_selector_and_partial_binding():
...
def test_arrow_preprocessor_attaches_to_data_module():
    configured = rf.Model(
        d_model=8,
        n_layers=1,
        n_heads=2,
...
def test_stratum_aware_selector():
    configured = rf.Model(
        d_model=8,
        n_layers=1,
        n_heads=2,
...
def test_arrow_selector_is_atomic_for_a_branch():
    configured = rf.Model(
        d_model=8,
        n_layers=1,
        n_heads=2,
...
def test_shared_selector_can_drive_different_leaf_effects():
    configured = rf.Model(
        d_model=8,
        n_layers=1,
        n_heads=2,
...
def test_nested_selectors_preserve_all_null_schema(preprocess, datatype):
...
def test_observation_selector_broadcasts_into_nested_records():
    source = pa.Table.from_pylist(
        [
            {
                "events": [
...
def test_query_backed_target_requires_its_source():
    configured = rf.Model(
        d_model=8,
        n_layers=1,
        n_heads=2,
