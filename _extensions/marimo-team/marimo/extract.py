...
class MarimoTableHTMLParser(HTMLParser):
    """Extract attributes from the first marimo-table tag in rendered HTML."""
    ...
    def handle_starttag(
        ...
        if tag == "marimo-table" and self.attrs is None:
            self.attrs = {key: value or "" for key, value in attrs}


def loads_json_attr(value: str | None) -> Any:
    """Decode marimo element attributes that are sometimes JSON-in-JSON."""
    decoded: Any = value or ""
    for _ in range(2):
...
def contains_marimo_element(markup: str) -> bool:
    return re.search(r"</?marimo[-\w]*\b", markup) is not None


def marimo_table_to_static_html(markup: str) -> str | None:
    parser = MarimoTableHTMLParser()
    parser.feed(markup)
    attrs = parser.attrs
    if attrs is None:
        return None
    ...
    try:
        rows = loads_json_attr(attrs.get("data-data"))
    ...
    try:
        field_types = loads_json_attr(attrs.get("data-field-types"))
    except json.JSONDecodeError:
        field_types = []
    ...
    columns = [
        ...
        if isinstance(field_type, list) and field_type and isinstance(field_type[0], str)
    ]
    if not columns and rows and isinstance(rows[0], dict):
        columns = [str(column) for column in rows[0].keys()]

    if not columns:
        return '<p class="qmo-static-empty">No rows</p>'
    ...
    body = "".join(body_rows)
    return (
        '<div class="qmo-static-table-wrap">'
        '<table class="qmo-static-table">'
...
def static_html_from_mime(mimetype: str, data: Any) -> str | None:
    if mimetype == "text/html":
        markup = str(data)
        table = marimo_table_to_static_html(markup)
        if table is not None:
            return table
        if contains_marimo_element(markup):
            return None
        return markup
    ...
    if mimetype == "application/vnd.marimo+mimebundle":
        ...
        if "text/html" in bundle:
            markup = str(bundle["text/html"])
            if contains_marimo_element(markup):
                return None
            return markup
        if "text/markdown" in bundle:
            return f"<pre>{escape(str(bundle['text/markdown']))}</pre>"
        if "text/plain" in bundle:
            return f"<pre>{escape(str(bundle['text/plain']))}</pre>"
        for key in ["image/svg+xml", "image/png", "image/jpeg"]:
            if key in bundle:
                if key == "image/svg+xml" and str(bundle[key]).lstrip().startswith("<"):
                    return str(bundle[key])
                return f'<img src="{escape(str(bundle[key]))}" alt="" />'

    if mimetype == "text/plain":
        return f"<pre>{escape(str(data))}</pre>"
    if mimetype == "application/json":
        return f"<pre>{escape(str(data))}</pre>"
    if mimetype == "image/svg+xml" and str(data).lstrip().startswith("<"):
        return str(data)
    if mimetype.startswith("image"):
        return f'<img src="{escape(str(data))}" alt="" />'
...
def get_mime_render(
    ...
    output = stub.output
    static_output = is_enabled(config.get("marimo-static-output"))
    render_options = {
    ...
    if output:
        ...
        if show_output and static_output and not mime_sensitive:
            static_html = static_html_from_mime(mimetype, output.data)
            if static_html is not None:
                return {
                    "type": "html",
                    "value": (f'<div class="qmo-static-cell-output">{static_html}</div>'),
                    **render_options,
                }
        ...
        if show_output and mime_sensitive:
            ...
            if mimetype == "application/vnd.marimo+mimebundle":
                try:
                    bundle_data = json.loads(output.data) if isinstance(output.data, str) else output.data
    ...
    # Nothing extra to display. If echo=true, Quarto will render the source
    # block from render_options; avoid emitting a second marimo editor copy.
    if not show_output or output is None:
        return {"type": "html", "value": "", **render_options}

    # HTML as catch all default for outputs that require the marimo runtime.
    return {
        "type": "html",
        "value": stub.render(
            display_code=False,
...
def build_export_with_mime_context(
    ...
    def tree_to_pandoc_export(root: Element) -> SafeWrap:  # type: ignore[valid-type]
        ...
        stubs: list[tuple[dict[str, bool], Optional[MarimoIslandStub | _ParseError]]] = []
        for child in root:
        ...
        static_output = is_enabled(global_options.get("marimo-static-output"))
        needs_runtime = any("<marimo-" in str(output.get("value", "")) for output in outputs)

        header = ""
        if needs_runtime or not (static_output and not mime_sensitive):
        ...
        if not mime_sensitive and (needs_runtime or not static_output):
            script_metadata = pyproject_to_script_metadata(str(global_options.get("pyproject", "")))
            notebook_code = build_export_notebook_code(app, script_metadata)
            header = render_export_context_script(notebook_code) + render_hidden_marimo_code(notebook_code) + header
...
if __name__ == "__main__":
    ...
    static_output = sys.argv[4].lower() == "yes" if len(sys.argv) == 5 else bool(default_config["marimo-static-output"])
    ...
    default_config["marimo-static-output"] = static_output
