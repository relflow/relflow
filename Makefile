...
render: ## Render the static docs site into docs/site/.
	$(QUARTO) render docs
	uv run --no-project --with pyyaml python proofs/render.py --clean
...
check-docs: ## Validate docs quietly without touching the workspace.
	@set -eu; \
		...
		rsync -a --exclude='__pycache__/' proofs/ "$$DOCS_CHECK_DIR/proofs/"; \
...
proofs: ## Run synthetic proofs of learned model behavior.
	uv run python proofs/run.py

build: render ## Alias for render.

clean: ## Remove rendered docs output and temporary proof inputs.
	uv run --no-project --with pyyaml python proofs/render.py --clean
	rm -rf docs/site
