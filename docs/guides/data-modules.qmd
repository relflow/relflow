...
| Option | Meaning |
| --- | --- |
| `model` | Supplies schema schema, `batch_size`, and tensorfield encoding state. |
...
A transformation preprocessor returns one wrapped observation for each input
observation:
...
the data module. Pipeline-provided parameters such as `strata`, `schema`, and
`encoding_context` are matched by name:
