...
`trainer.predict(...)`. Attach `jv.Writer(...)` to write each prediction batch
...
Writer postprocessors can request named runtime values such as:

| Parameter | Meaning |
| --- | --- |
| `input` | Encoded Lightning batch. |
| `batch` | Alias for the encoded batch. |
| `metadata` | Raw input metadata for each row. |
...
For more output-shaping patterns, see [Postprocessors](postprocessors.qmd).
...
trainer = lit.Trainer(
    accelerator="gpu",
    devices=1,
    callbacks=[jv.Writer("predictions")],
)
...
- Use [Data Modules](data-modules.qmd) to choose Polars or streaming inputs.
- Use [Postprocessors](postprocessors.qmd) to reshape outputs.
- Use [Learning Modes & Embeddings](../core-concepts/embeddings.qmd) to configure
  embedding outputs.
