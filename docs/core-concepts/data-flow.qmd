...
A JSON2Vec schema sits in the middle of the complete modeling path. It tells the
...
`jv.Observation`. A preprocessor may yield several observations from one raw
record, but each output is an independent example with its own singleton root.
Internally, JSON2Vec wraps each observation as a one-record list so inferred
...
JSON2Vec creates the masked target position and decodes it. Other learning roles
...
| Request | The configuration object created by `jv.Number`, `jv.Category(...)`, and the other leaf constructors. |
