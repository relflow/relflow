...
Internally, RelFlow wraps each observation as a one-record root before eager
Awkward coalescing; that wrapper does not create multiple root items.
...
The root name affects addresses, not source lookup. One eager Awkward
coalescing call prepares every encoded field in the processed batch. Each
directly bound leaf receives a column projection that becomes fixed shape,
final value state, retained flat values, and dense destinations.

A leaf with an explicit `query=...` instead evaluates that JMESPath expression
against the processed observation. Its result converges with direct values
before paired value/state regularization, so overflow, final `Tokens` state,
prediction literals, and dense placement do not fork into a second engine. All
fields are ready before datatype codecs begin. Queries are never inferred:
omitting `query` always chooses direct schema-address projection.
