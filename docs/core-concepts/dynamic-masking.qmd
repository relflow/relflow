...
`target=True`. Branch masks add one more control: they select slices of a
...
| `jv.Mask(...)` | `Branch` | Repeated branch coordinates, projected onto descendant leaves. |

Leaf masking is enough when every value can be sampled independently. Branch
masking is better when one repeated row should be hidden consistently across
several fields, or when the sampling window is defined by recency inside a
branch.
...
| `branch` | `False` | Use fixed padded branch coordinates instead of observed non-padded coordinates. |
