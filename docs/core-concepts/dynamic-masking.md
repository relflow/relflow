...
Use array masks when the masking rule is about a repeated coordinate rather
than one leaf value. Common examples are "mask two values from each word",
"mask half of the last 10 retained events", or "mask a few line items but leave
one field visible."
...
The tensorfield display shows the first batch item and the root singleton
dimension. Nested arrays are printed with one row per inner array:
