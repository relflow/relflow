...
`json2vec` infers one from the field name and its parent `Branch` nodes. When the
source payload does not match the schema names, pass an explicit
[JMESPath](https://jmespath.org/) expression with `query=...`.

Queries are the bridge between your raw record shape and the model tree. Branches
...
Changing the root branch name changes addresses, not source queries:
...
Use `Branch` when the source contains fields that share a repeated context:
...
the `line_items` dimension. Branch overflow is resolved after query selection,
using the order returned by the query. Use `overflow="tail"` on a `Branch` when
...
Default inference extends through every parent `Branch`:
...
model = jv.Model(
    ...
    accounts=jv.Branch(
        length=8,
        account_type=jv.Category(size=16),
...
| Address | Query |
| --- | --- |
| `record/accounts/account_type` | `[*].accounts[*].account_type` |
...
- The result shape should match the field's schema location. A leaf under one
  `Branch` should usually return nested lists with one array dimension.
...
Select leaf tensorfields when changing `query`. `Branch` nodes define model
...
model = jv.Model(
    ...
    label=jv.Category(query="[*].outcome", size=8),
)
...
## Custom Queries Under Branches

When a schema branch name differs from the source array path, set full explicit
...
model = jv.Model(
    ...
    line_items=jv.Branch(
        length=32,
        sku=jv.Category(query="[*].payload.items[*].product.sku", size=2048),
...
model = jv.Model(
    ...
    login_events=jv.Branch(
        length=16,
        device_id=jv.Entity(query="[*].events[?event_type == 'login'].device_id"),
        risk_score=jv.Number(query="[*].events[?event_type == 'login'].risk_score"),
    ),
)
...
Sibling fields under the same `Branch` should use the same filter so their values
remain aligned.
:::
...
model = jv.Model(
    ...
    n_layers=1,
    n_heads=4,
    first_origin=jv.Category(query="[*].legs[0].origin_airport", size=512),
...
an array dimension. Most fields under `Branch` should use `[*]` at each array
...
| Branch child values are misaligned | Different child queries filter or flatten differently | Use the same array path and filters for sibling fields. |
| Shape is flatter than expected | The query used `[]` flattening or skipped a `[*]` level | Use `[*]` at every schema branch level. |
