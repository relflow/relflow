...
names shared contexts for child fields, usually repeated collections such as
...
| Boolean | [`Boolean`](../data-types/boolean.qmd) | The source has more than two labels; use `Category`. |
...
| Local repeated identity | [`Entity`](../data-types/entity.qmd) | The ID is intentionally a small, stable, globally learned label; use `Category`. |
...
- `[{"name": "red"}, {"name": "sale"}]` is a `Branch` if each item has fields.
...
| `Boolean` | Yes, value plus true probability | Yes | Vocabulary-free binary decoding. |
...
| `Branch` | No direct payload | No | Child fields may emit predictions. |
...
options unique to that tensorfield. `Branch` has structural options because it
...
| `nullable` | `True` | Allows explicit `None` input values. Set `False` to raise during encoding whenever this leaf contains `state == Tokens.null`. |
...
`p_mask`, `p_prune`, and `target` are leaf tensorfield options. `Branch` nodes
define repeated structure and can emit embeddings. They can also own
`jv.Mask(...)` policies that select repeated coordinates and apply that
...
- `padded`: the configured branch shape has a slot with no source value.
...
null field state. A missing repeated slot inside a `Branch` is padded.
If a leaf is configured with `nullable=False`, encoding raises when the extracted
input contains an explicit null for that leaf.
...
say "not a number", but it cannot distinguish null source data, padded branch
