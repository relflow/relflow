...
recency. The login-session branch and root node request embeddings so offline
analysis can inspect both customer-level and login-history representations.

`customer_id` stays in the raw and processed records for joins, audit trails,
or request metadata, but it is deliberately not modeled as a `Category`.
Treating customer identifiers as a large categorical vocabulary is usually a
memory-heavy anti-pattern unless the global identity itself is intentionally
part of the model.
...
login_embedding = predictions[jv.Address("customer", "login_sessions")]["embedding"]
```
