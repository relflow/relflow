...
nested branches. The goal is not model quality. It is to make the package's core
loop concrete.
...
Use the package root import. `Model.from_tree(...)` turns the field
...
The Rich display is the fastest way to verify the tree that was built: root branch,
...
| branch `mask=` | repeated coordinates are hidden across descendant leaves | decoded reconstruction for affected trainable leaves |
...
`jv.Model` is a LightningModule, and `jv.PolarsDataModule` is a
...
padded branch slots, and training masks are distinct.
...
signal. Use `Branch` when a record contains a list of child objects:
...
model = jv.Model.from_tree(
    ...
    measurements=jv.Branch(
        jv.Category("name", size=16),
        length=8,
        embed=True,
        value=jv.Number,
    ),
    diagnosis=jv.Category(target=True, size=2),
)
...
nested_datamodule = jv.PolarsDataModule(
    model=model,
...
diagnosis = nested_predictions[jv.Address("record", "diagnosis")]
measurements = nested_predictions[jv.Address("record", "measurements")]
