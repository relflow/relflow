...
`datetime.strptime` first. `None` is encoded as a null state, and missing branch
...
- `content`: a `(sin, cos)` pair on the unit circle for each configured date
  part, treating each part as a continuous angular coordinate at its canonical
  period.

The per-datepart loss is the cosine distance between the predicted and target
unit-circle vectors, and the reported `mae` is the mean absolute angular error
in radians. The state loss is added to the averaged datepart losses.
