...
| Hierarchical context encoding | [Model Tree](core-concepts/model-tree.qmd), [Branch](data-types/branch.qmd) |
...
Leaf fields become typed tensorfield requests. Branch nodes define shared
contexts, optionally apply local attention, and pool those contexts upward.
Targets describe values the model should learn to reconstruct or predict.
Embedding settings describe which nodes should expose intermediate
representations.
...
model = jv.Model(
    ...
    embed=True,
    alcohol=jv.Number,
    malic_acid=jv.Number,
    color_intensity=jv.Number,
    proline=jv.Number,
    cultivar=jv.Category(target=True, size=4),
)
...
model.update(jv.where("name") == "cultivar", target=True)
model.update(jv.where("name") == "proline", active=False)
```
